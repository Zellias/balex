...
### 10.3 Complete List of 62 Official Bale Bot API Methods
All 62 official methods from `https://docs.bale.ai/` are natively available on `BaleBot`:
...
| **Edit & Delete** | `editMessageText(chatId, messageId, text, options)`, `editMessageCaption(chatId, messageId, caption, options)`, `editMessageReplyMarkup(chatId, messageId, replyMarkup)`, `deleteMessage(chatId, messageId)`, `deleteMessages(chatId, messageIds)` |
